...
@section('link')
<li class="breadcrumb-item "><a href="{{ route('progres-maket.index') }}">Maket</a></li>
...
<div class="col-12">
    <div class="card">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form action="{{ route('progres-maket.update', $progres->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    <div class="col-6">
                        ...
                        <div class="form-group">
                            ...
                            <select name="pesanan_id" id="pesanan_id" class="custom-select form-control-border border-width-2">
                                <option value="{{ $progres->pesanan->id }}">{{ $progres->pesanan->portofolio->name }} pesanan dari {{ $progres->pesanan->user->name }}</option>
                            </select>
                        </div>
                        <div class="form-group">
